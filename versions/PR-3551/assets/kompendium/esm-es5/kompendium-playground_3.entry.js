...
var Playground = /** @class */ (function () {
    function Playground(hostRef) {
         ...
         */
        this.component = {};
        this.activateTab = function (id) { return function () {
            _this.activeTab = id;
        }; };
    ...
    Playground.prototype.render = function () {
        ...
        var sources = this.component['sources'] || [];
    ...
    Playground.prototype.renderTabs = function (sources) {
        return sources.map(this.renderTab);
    };
    Playground.prototype.renderTab = function (source, index) {
        var classList = {
            tab: true,
    ...
    Playground.prototype.renderItems = function (sources) {
        return sources.map(this.renderItem);
    };
    Playground.prototype.renderResult = function () {
        var ExampleComponent = this.component.tag;
        var text = '##### ' + this.component.docs;
        return (h("div", null, h("kompendium-markdown", { text: text }), h(ExampleComponent, null)));
    };
    Playground.prototype.renderItem = function (source, index) {
        var classList = {
            'tab-item': true,
            active: this.isTabActive(source, index),
        };
        var code = source.source.replace(/\/\*\*.+?\*\//gms, '');
        return (h("kompendium-code", { random: Math.random(), class: classList, language: source.type }, code));
    };
    Playground.prototype.isTabActive = function (source, index) {
        var isActive = this.activeTab === source.filename;
        if (!isActive) {
...
var taglistCss = ":host{display:block}code{margin-bottom:0.5rem;margin-left:0.5rem}img{width:1rem;-webkit-filter:invert(0.5) sepia(1) hue-rotate(185deg) saturate(5);filter:invert(0.5) sepia(1) hue-rotate(185deg) saturate(5);vertical-align:middle}.tag--deprecated img{-webkit-filter:invert(0.35) sepia(1) hue-rotate(-60deg) saturate(5);filter:invert(0.35) sepia(1) hue-rotate(-60deg) saturate(5)}";
