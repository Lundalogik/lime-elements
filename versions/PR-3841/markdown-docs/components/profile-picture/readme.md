...
| `resize`     | --            | Optional client-side resize before emitting the file. If provided, the selected image will be resized on the client device. :::note HEIC may not decode in all browsers; when decoding fails, the original file will be emitted. See the examples for more info. ::: | `{ width: number; height: number; fit?: "cover" \| "contain"; type?: "image/jpeg" \| "image/png"; quality?: number; rename?: (originalName: string) => string; }` | `undefined`                                               |
