...
| `image`      | `image`      | A picture to be displayed instead of the icon on the chip.                                                                                                                                                                | `Image`                                                                | `undefined`           |
...
| `link`       | `link`       | If supplied, the chip will become a clickable link.                                                                                                                                                                       | `{ title?: string; target?: string; href: string; rel?: string; }`     | `undefined`           |
...
| `menuItems`  | `menu-items` | When provided, the chip will render an ellipsis menu with the supplied items. Also, this will hide the "remove button" when `removable={true}`, as the remove button will automatically become the last item in the menu. | `(ListSeparator \| MenuItem<any>)[]`                                   | `[]`                  |
