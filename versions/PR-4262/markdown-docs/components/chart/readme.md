...
| `maxValue`              | `max-value`               | Sets the upper bound used to scale item values. For `stacked-bar`, `pie`, `doughnut`, and `ring`, it defines the whole and defaults to the combined item values or range extents. For `bar`, `dot`, `area`, and `line`, it is the requested upper bound. The chart can round that bound up to the next `axisIncrement`. Has no effect on `nps` or `scatter`. | `number`                                                                                                     | `undefined`     |
