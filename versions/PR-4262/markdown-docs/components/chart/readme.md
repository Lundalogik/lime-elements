...
| `hideItemPercentage`    | `hide-item-percentage`    | Hides the percentage that a chart item's tooltip appends to its `text`. The percentage is the item's share of the chart's range, which is only meaningful when that range is a whole, such as a total or a capacity. Has no effect on the `bar`, `dot`, `nps`, and `scatter` types, whose tooltips never show a percentage. | `boolean`                                                                                                    | `false`         |
...
### Used by
 ...
 - [limel-example-chart-hide-item-percentage](examples)
...
graph TD;
  ...
  limel-example-chart-hide-item-percentage --> limel-chart
