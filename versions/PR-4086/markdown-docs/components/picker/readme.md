...
| `allItems`             | --                       | Only used if no `searcher` is provided. The picker will then use a default search function that filters the `allItems` based on the `text` and `secondaryText` properties of the items. This way, custom search functions are typically only needed when the search is done on the server. For performance reasons, the default searcher will never return more than 20 items, but if there are more than 20 items, the rest can be found by typing more characters in the search field. | `PickerItem<PickerValue>[]`                                                | `[]`        |
...
| `emptyResultMessage`   | `empty-result-message`   | A message to display when the search returned an empty result.  If unset, the picker shows a default translated message ("No matching results" in English) chosen by the `language` prop. Set this to override the default with custom text — for example, when the picker's domain calls for more specific wording like "No matching participants found".                                                                                                                               | `string`                                                                   | `undefined` |
...
| `language`             | `language`               | Defines the language for translations. Affects the labels rendered by the picker itself, such as the "Results matching" header shown above the suggestion list while the user is typing.                                                                                                                                                                                                                                                                                                 | `"da" \| "de" \| "en" \| "fi" \| "fr" \| "nb" \| "nl" \| "no" \| "sv"`     | `'en'`      |
...
| `searcher`             | --                       | A search function that takes a search-string as an argument, and returns a promise that will eventually be resolved with an array of `ListItem`:s.  See the docs for the type `Searcher` for type information on the searcher function itself.                                                                                                                                                                                                                                           | `(query: string) => Promise<(ListSeparator \| PickerItem<PickerValue>)[]>` | `undefined` |
| `value`                | --                       | Currently selected value or values. Where the value can be an object.                                                                                                                                                                                                                                                                                                                                                                                                                    | `PickerItem<PickerValue> \| PickerItem<PickerValue>[]`                     | `undefined` |
...
| `change`   | Fired when a new value has been selected from the picker | `CustomEvent<PickerItem<PickerValue> \| PickerItem<PickerValue>[]>` |
| `interact` | Fired when clicking on a selected value                  | `CustomEvent<PickerItem<PickerValue>>`                              |
...
### Used by
 ...
 - [limel-example-picker-empty-result-message](examples)
 ...
 - [limel-example-picker-non-removable](examples)
...
graph TD;
  ...
  limel-example-picker-empty-result-message --> limel-picker
  ...
  limel-example-picker-non-removable --> limel-picker
