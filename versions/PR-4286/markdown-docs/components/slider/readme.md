...
| `factor`                   | `factor`                     | Default value: 1. The factor that the properties `value`, `valuemax`, `valuemin`, and `step` are multiplied by. On `change` divides the value by the factor, so the original format stays the same.                                                  | `number`                                                               | `1`         |
...
| `language`                 | `language`                   | Defines the language for translations of the accessible labels.                                                                                                                                                                                      | `"da" \| "de" \| "en" \| "fi" \| "fr" \| "nb" \| "nl" \| "no" \| "sv"` | `'en'`      |
...
| `required`                 | `required`                   | Set to `true` to indicate that the slider is required. A required slider does not offer the clear button, since a required value cannot be unset. It can still be initialized unset (with a non-finite `value`) to prompt the user to make a choice. | `boolean`                                                              | `false`     |
...
| `value`                    | `value`                      | The value of the input. Set it to `null` to leave the slider unset, which is also what the `change` event emits once the value is cleared. Any other value that is not a finite number — `undefined`, or `NaN` — unsets the slider too.              | `number`                                                               | `undefined` |
| `valuemax`                 | `valuemax`                   | The maximum value allowed                                                                                                                                                                                                                            | `number`                                                               | `100`       |
| `valuemin`                 | `valuemin`                   | The minimum value allowed                                                                                                                                                                                                                            | `number`                                                               | `0`         |
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------- |
| `change` | Emitted when the value has been changed. Emits `null` when the value has been cleared and the slider becomes unset, so handlers must account for a value that is not a number. | `CustomEvent<number>` |
...
### Used by
 ...
 - [limel-example-slider-unset](examples)
