...
### Headings in documentation examples

When an example renders a top-level heading or caption, use `<h2 class="h3">` (or `class="h4"`, etc.) so the heading level fits the docs page's document outline while the visual size can be chosen freely. The classes are defined in [`src/examples/heading-classes.scss`](src/examples/heading-classes.scss) — see that file for the full rationale — and are imported into the example's shadow root with `@use '../../../examples/heading-classes';` in the example's `.scss` file. Bare `<h3>` and deeper headings in examples will fail the accessibility test suite's `heading-order` rule (they skip a level after the page's `<h1>`).
