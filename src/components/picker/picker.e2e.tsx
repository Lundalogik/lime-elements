...
/**
 * Flushes renders until `isSettled` holds. The number of passes between
 * "the searcher resolved" and "the suggestion text is in the DOM" varies
 * with machine load, so it cannot be fixed.
 *
 * Throws rather than returning on timeout, so a wait that never succeeded
 * is reported as such instead of as whichever assertion happens to fail
 * next.
 *
 * @param what - what is being waited for, named for the failure message
 * @param isSettled - whether it has happened
...
async function pollUntil(
    ...
    if (!isSettled()) {
        throw new Error(
            `Timed out after ${POLL_TIMEOUT}ms waiting for ${what}. ` +
                `Suggestions: ${JSON.stringify(getSuggestionTexts())}`
        );
    }
}
...
describe('limel-picker', () => {
    ...
    describe('when picking an item from a multi-pick list', () => {
        describe('by default', () => {
            ...
            it('drops the picked item once the consumer has re-rendered', async () => {
                const { root, waitForChanges } = await render(
                    <limel-picker
                        label="Pick"
                        multiple={true}
                ...
                root.addEventListener('change', (event: Event) => {
                    const picked = (event as CustomEvent)
                ...
                expect(getSuggestionTexts()).toEqual(['Blueberry', 'Cherry']);
            });
    ...
    describe('when Esc clears the query', () => {
        it('replaces the results at once, without waiting out the debounce', async () => {
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
                    multiple={true}
            ...
            expect(getSuggestionSeparatorTexts()).toEqual([
                'Results matching "black"',
            ]);

            getSearchInput(root).dispatchEvent(
                new KeyboardEvent('keydown', {
                    key: 'Escape',
                    bubbles: true,
                    composed: true,
            ...
            expect(getSuggestionTexts()).toEqual([
                expect.stringContaining('Blueberry'),
    ...
    describe('when a chip is removed while searching', () => {
        ...
        it('goes back to the default suggestions', async () => {
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
                    multiple={true}
        ...
        it('does not reopen a dropdown the user dismissed', async () => {
            const { queries, searcher } = recordingSearcher();
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
