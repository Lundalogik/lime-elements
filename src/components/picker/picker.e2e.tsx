...
/**
 * Mirrors emitted values back, but only on the next task, the way a
 * consumer that batches state updates does — React's `setState`.
 *
 * @param root - the picker under test
 */
function echoValueChangesOnNextTask(root: HTMLLimelPickerElement) {
    root.addEventListener('change', (event: Event) => {
...
describe('limel-picker', () => {
    ...
    describe('when picking an item from a multi-pick list', () => {
        describe('by default', () => {
            ...
            it('keeps the picked item out of them even when the consumer echoes `value` late', async () => {
                const { root, waitForChanges } = await render(
                    <limel-picker
                        label="Pick"
                        multiple={true}
                ...
                echoValueChangesOnNextTask(root);
                await waitForChanges();
                await focusAndType(root, 'black', waitForChanges);
            ...
            it('keeps the first pick when the consumer echoes `value` late', async () => {
                const { root, waitForChanges } = await render(
                    <limel-picker
                        label="Pick"
                        multiple={true}
                ...
                root.addEventListener('change', (event: Event) => {
                    const detail = (event as CustomEvent).detail;
                    emitted.push(detail);
                    // Slower than the picker's own timings, the way a
                    // consumer waiting on a server round trip is.
                    setTimeout(() => {
                        root.value = detail;
                    }, 300);
                });
                await waitForChanges();
                await focusAndType(root, 'b', waitForChanges);

                await pickSuggestion('Blackberry', waitForChanges);
                await pickSuggestion('Blueberry', waitForChanges);
    ...
    describe('when the consumer declines the change', () => {
        it('defers to `value` again once the session ends', async () => {
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
                    multiple={true}
            ...
            getSearchInput(root).blur();
            await waitOutDebounce(waitForChanges);

            await chipSet.setFocus();
            await waitForSuggestions(waitForChanges);
            await pickSuggestion('Cherry', waitForChanges);
        ...
        it('keeps a pick made before blurring, once `value` has caught up', async () => {
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
                    multiple={true}
    ...
    describe('when a chip is removed while searching', () => {
        ...
        it('brings the removed item back even when the consumer echoes `value` late', async () => {
            const { root, waitForChanges } = await render(
                <limel-picker
                    label="Pick"
                    multiple={true}
            ...
            expect(
                getSuggestionTexts().some((text) => text.includes('Cherry'))
            ).toBe(true);
        });
