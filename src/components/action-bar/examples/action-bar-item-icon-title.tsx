 ...
 * for the users.
 *
 * However, some designs try to make a cleaner and more minimal user interface,
 ...
 * this combination of icon and text will be interpreted as "Add New Todo"
 * - **🔄 icon followed by "List"**: For a sighted user,
 * this combination of icon and text will be interpreted as "Refresh List"
 *
 * For unsighted users, the text is enough to understand the meaning of the action,
 ...
 *
 * In this example, we have added a more descriptive `title` property to the icons,
 * so that the screen readers can interpret the icons as words, and provide a better
 * user experience for unsighted users. Also notice how hovering the item would
 * display a tooltip that is generated from the combination of the `title` and `text`
    ...
    public render() {
        return [
            <div class="application has-floating-action-bar is-resizable">
                <limel-list items={this.listItems} class="has-striped-rows" />
                <limel-action-bar
