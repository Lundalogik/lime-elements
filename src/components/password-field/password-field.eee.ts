...
describe('limel-password-field', () => {

    it('should render my component', async () => {
        const page = await newSpecPage({
            components: [PasswordField],
            html:  `<limel-password-field
                        label="Password"
                    ></limel-password-field>`,
        });

        expect(page.root).toEqualHtml(`
        <limel-password-field></limel-password-field>
        `);
    });
    ...
    describe(`with type="Password"`, () => {
        ...
        beforeEach(async () => {
            page = await createPage(`
                ...
                ></limel-password-field>
            `);
            limelInput = await page.find('limel-password-field');
            inputContainer = await page.find(
                'limel-password-field>>>label.mdc-text-field'
            );
        });
