...
describe('split view helpers', () => {
    it('produces split hunks with collapsedBefore', () => {
        ...
        const regrouped = regroupLines(result.allLines, 2);
        expect(regrouped.hunks.length).toBeGreaterThanOrEqual(1);

        // First hunk should have collapsedBefore since change is at line 10
        const hunkWithCollapsed = regrouped.hunks.find(
    ...
    it('converts hunk lines to paired split rows', () => {
        ...
        const rows = buildSplitLines(hunk.lines);
