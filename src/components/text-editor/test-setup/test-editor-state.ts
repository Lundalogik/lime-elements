...
/**
 * Creates a simple empty document with optional text content.
 *
 * @param text - Optional text to include in the document
 * @param schema - Optional custom schema (uses createTestSchema by default)
 * @returns An EditorState with a simple document
 */
export function createDocumentWithText(
    ...
    return createEditorState(content, schema);
}
